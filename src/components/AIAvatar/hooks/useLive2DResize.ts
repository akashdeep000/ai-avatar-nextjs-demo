...
export const applyScale = (scale: number) => {
    try {
        ...
        console.debug('Model not ready for scaling yet', error);
    }
};
...
export const useLive2DResize = ({
    ...
    const handleResize = useCallback(() => {
        ...
        try {
            ...
            if (delegate) {
                if (delegate._view?._programId && canvasRef.current) {
                    delegate.onResize();
                } else {
                    console.warn('[Resize] Delegate or view not ready, skipping resize.');
                }
            } else {
